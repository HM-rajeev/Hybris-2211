...
public class HmDefaultCreditCardPaymentInfoCreateStrategy implements HmCreditCardPaymentInfoCreateStrategy
{
			...
			final SubscriptionInfoData subscriptionInfo, final PaymentInfoData paymentInfoData, final boolean saveInAccount)
	{
		...
		{
			billingAddress.setRegion(getCommonI18NService().getRegion(country, country.getIsocode() + "-" + customerInfoData.getBillToState()));
		}
		final String email = getCustomerEmailResolutionService().getEmailForCustomer(customerModel);
		billingAddress.setEmail(email);

		final CreditCardPaymentInfoModel cardPaymentInfoModel = this.createCreditCardPaymentInfo(subscriptionInfo, paymentInfoData, billingAddress, customerModel, saveInAccount);
